...
using ServicingService;
using ServicingService.Repositories;

var builder = WebApplication.CreateBuilder(args);

builder.Logging.AddLoanPlatformLogging();
TraceBuffer.ServiceName = "Servicing"; // tags every TraceLogger.Trace() call from this process for the UI's Logs tab
...
#if USE_SQL_SERVER
builder.Services.AddDbContext<ServicingService.Data.ServicingDbContext>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("ServicingDb")));
...
{
    var db = migrationScope.ServiceProvider.GetRequiredService<ServicingService.Data.ServicingDbContext>();
